...
def is_trajectory(nc):
    '''
    Returns true if the dataset has a trajectory feature
    '''
    try:
        feature_type_global = nc.featureType

        latitude = nc.variables[get_lat_variable(nc)]
        longitude = nc.variables[get_lon_variable(nc)]
    except KeyError:
        return False

    return latitude.dimensions == ('time',) and longitude.dimensions == ('time',) and feature_type_global == 'trajectory'


def get_geojson_trajectory(nc):
    '''
    Returns a GeoJSON LineString for the trajectory coordinates
    '''
    ...
    coords = []
    for lon, lat in zip(longitude.tolist(), latitude.tolist()):
...
def get_shape(nc):
    '''
    Returns the shapely shape of the dataset
    :param netCDF4.Dataset nc: netCDF dataset
    '''
    from shapely.geometry import Point, box

    geo_extents = get_geo_extents(nc)
    if geo_extents[0] == geo_extents[1]:
        return Point(geo_extents[0][0], geo_extents[0][1])
    return box(geo_extents[0][0], geo_extents[0][1], geo_extents[1][0], geo_extents[1][1])