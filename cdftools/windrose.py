...
Original code forked from:
    ...
    windrose 1.4 by Lionel Roubeyrie
...
class WindroseAxes(PolarAxes):
    ...
    def set_radii_angle(self, **kwargs):
        ...
        radii_labels = ["%d%%" % r for r in radii]
    ...
    def get_radii(self):
        '''
        Return a set of sensible radii for the rose plot
        '''

        maxval = self.get_rmax()
        if maxval < 5:
            radii = [0.1, 1, 2, 3, 4, 5]
        elif maxval < 10:
            radii = [0.1, 2, 4, 6, 8, 10]
        elif maxval < 15:
            radii = [0.1, 5, 10, 15]
        elif maxval < 20:
            radii = [0.1, 4, 8, 12, 16, 20]
        elif maxval <= 25:
            radii = [0.1, 5, 10, 15, 20, 25]
        elif maxval <= 50:
            radii = [0.1, 10, 20, 30, 40, 50]
        elif maxval <= 75:
            radii = [0.1, 15, 30, 45, 60, 75]
        elif maxval <= 100:
            radii = [0.1, 20, 40, 60, 80, 100]

        return np.array(radii)

    def _update(self, rmax=None):
        if rmax is None:
    ...
    def legend(self, loc='lower left', **kwargs):
        ...
        def get_labels():
            ...
            for i in range(nbins):
                if last_bin <= 3:
                    labels.append("{:0.1f} - {:0.1f}".format(bins[i], bins[i + 1]))
                else:
                    labels.append("{:0.0f} - {:0.0f}".format(bins[i], bins[i + 1]))
            # Add + symbol on the last bin
            labels[-1] += '+'
    ...
    def contourf2(self, dir, var, **kwargs):
        ...
        * cmap : a cm Colormap instance from matplotlib.cm.
        ...
        for i in range(nbins):
            ...
            offset += vals[i, :]
            zorder = ZBASE + nbins - i
            # From https://github.com/python-windrose/windrose/pull/114/files
            patch = self.fill(np.append(angles, 0), np.append(val, 0), facecolor=colors[i],
...
if __name__ == '__main__':
    from pylab import figure, show, setp, random, draw
    vv = random(500) * 6
    dv = random(500) * 360
